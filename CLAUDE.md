...
- ザミャーチン『われら』(`we`, ru) を追加（2026-08-16）: PD確認（没1937 → 日本は1988年から、露は既に満了、
  戦時加算はソ連に適用なし）→ 原文40記録・1,544段落を取得、カタログ・台帳（people/style-guide）作成、
  記録一（10段落）だけ訳して文対訳までを実測。**残り39章（we-02〜we-40）が未翻訳**。
  推定 日本語15万字（罪と罰の約1/4）。章数は40で罪と罰の41章とほぼ同じなので 1章=1エージェント方式がそのまま乗る。
  一括翻訳の前に記録一の訳文をレビューし、style-guide の `people.*.firstPerson` と `names` を確定させること
