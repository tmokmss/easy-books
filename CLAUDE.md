...
- チェーホフ短編集（`chekhov`, ru）**全10編を完成**（2026-09-13）。**初の「短編集＝1作品」構成**。
  1 workId に短編を章として並べるので、書棚には背表紙が1本だけ増え（「全10章・文庫換算 約89ページ」）、
  構造ビューがそのまま目次になる（`chapterLabel` に短編名）。部を持たないので `overviews/chekhov.json` は無い。
  収録は「悲しみ」を巻頭に置き、残りを執筆年順に並べた:
  悲しみ(1886) / 役人の死(1883) / ふとっちょとやせっぽち(1883) / カメレオン(1884) / かき(1884) /
  たくらむ者(1885) / ヴァーニカ(1886) / 眠たい(1888) / 学生(1894) / いとしい人(1899)。
  **全10編・430段落・露語13,564語 → 日本語44,510字（文庫換算 約89ページ）、文対訳1,278セグメント。**
  訳文は claude-opus-5 が5編（悲しみ・ヴァーニカ・眠たい・学生・いとしい人）、
  gpt-5.6-sol が5編（Codex レーン併用）。全段落 `draft`。
  台帳は people 31人 / names 95語 / terms 126語 / rules 21項目。
  階層ズームの要約レイヤーは節44・段落要約160（すべて machine-checked、FAIL 0）。
  読解支援レイヤーは語注290・人物46・参照15 の計351、glossary 244項目すべて `verified: false`。
  PDは没1904で全法域クリア（戦時加算の検討も不要）。底本は「眠たい」以外すべて
  ПСС в 30 томах（Наука）の当該巻に拠る ru.wikisource のテキストで、編ごとのリビジョンは
  works.json の `sourceRef` に列挙した（「眠たい」だけ底本の書誌が示されていない＝要確認）。
  **訳の要点**: ①チェーホフは現在形で全編を通す作品が多く（悲しみ・カメレオン・眠たい）、
  これが場面を目の前で進行させる仕掛けなので訳文も現在形で通す。過去形の作品は過去形で訳す
  ②`горбач` は「背の曲がった男」で統一（「せむし」は現代日本語では差別語）
  ③二人称は ты→「おまえ」で固定。ただし九歳が祖父に書く手紙（ヴァーニカ）のように
  日本語として成り立たない箇所は、代名詞を使わず呼びかけ（「おじいさん」）で受ける
  ④子どもの一人称はひらがな「ぼく」（かき・サーシャ）
  ⑤聖書由来の固有名は日本語の慣用表記（使徒ペテロ／イエス）。同語の皇帝名とは訳語を分ける
  （апостол Пётр→ペテロ／Пётр→ピョートル）
  次工程は glossary 244項目の裏取り＋`verified: true` 化と、draft→checked の人間レビュー
- **短編集を1冊として扱うときに詰まる3点**（チェーホフ短編集で実測）:
  ①**底本が作品単位**。`sourceRef` は works.json にしかなく章スキーマに無い。編ごとに Wikisource の
    ページ・リビジョンが違うので、当面は `sourceRef` に編ごとの底本を列挙する（10編で845字になった）。
    編数がさらに増えたら章スキーマに任意の `sourceRef` を足すほうが安い
  ②**語注台帳も作品単位なので、同じ id が編をまたいで共有される。** `body` に「ここでは〜」と
    編固有の文脈を書くと噛み合わない。共有されやすい語（通貨・度量衡・官等・器物）は語そのものの
    説明だけにし、定義は1つのフラグメントに置いて他の編は id を参照するだけにする。
    10編で共有された id は7つ（barin / traktir / kaloshi / pech / ruble / versta / excellency）。
    `ruble` は「ルーブル」と「十ルーブル」で `term` が割れてマージが落ちた
    → **`term` は見出し語（語の基本形）にする。本文中の表示文字とは別でよい**
  ③**人物 id は編をまたいで重複させない。** 同名人物（Пелагея が「眠たい」と「ヴァーニカ」の両方に出る）は
    別 id にするか、脇役なら登録しない。style-guide の `people.*`（口調）も同じ名前空間なので、
    役どころだけの話者は編を示す接尾辞をつける（`doctor_spat` / `khozyain_spat` / `boy_ustritsy`）
  ④**読書画面が長編前提で作られているので、名前空間が1つだと2箇所壊れる。**
    `works.json` の `anthology: true` で切り替える（2026-09-13 追加）:
    - 登場人物レール（`PersonRail`）は作品の人物を**全員**並べる設計（長編の配役表として意図的）。
      短編集では他の編の人物まで並ぶ（「かき」は登場人物0人なのに31人並んでいた）。
      `anthology` ではその章の `{{p:}}` / `{{r:}}` が参照する id だけに絞り、0人なら帯を出さない
      （帯が消えるとデスクトップのグリッドで本文が 13rem の列に潰れるので `.no-rail` で1カラムに戻す）
    - 「ここまでのあらすじ」は前の章の章要約を出す。短編集では**無関係な編のあらすじが並び、
      他編の結末をばらす**（「学生」で「8章分」と出ていた）。`anthology` では出さない
    glossary は一覧UIが無く（タップで引くだけ）作品単位の名前空間でも読者には見えないが、
    共有される語の書き方だけは②の注意が要る
- **一括翻訳は「1編で台帳を固めてからファンアウト」が効く（実測）**: 最初の1編で style-guide を確定し、
  残り9編を9レーン（Codex 5・Claude 4）に流し、各レーンの「新規」報告を台帳に還流してから検査した。
  章単位の割れは**3件だけ**で済んだ（`шилишпер` の ш→ス 誤写／`Ванька` の章題と本文の割れ／
  `Иван` の長音落ち）。罪と罰（台帳を固めずに41章を流して事後修正500箇所）との差がそのまま順序の効果
- **Codex レーンの注意**: `--by <label>` を渡してマージしても、そのあとエージェントが自前スクリプトで
  章JSONを書き直すと `translatedBy` が落ちることがある（「ふとっちょとやせっぽち」で発生。
  注釈適用後だったので再マージできず、フィールドだけ手で補った）。
  ファンアウト後は `translatedBy` の欠落を必ず確認する
...
- check-alignment の既知の誤検知: 複合数詞（семьсот тридцать 等）、日本語の自然な文分割による文数乖離（大半は良性）、
  **訳文の撥音便の否定**（「知らん」「分からん」「着かん」。jaNegations が ない/ぬ/ず しか見ないので
  ...
  日本語に否定は現れない）、**数詞でない `один/одна`**（«подлость одна только»＝「ただの〜」、
  «смерть одна»＝「死ぬばかり」。「1」が訳文に無いと出る）。
  チェーホフ短編集の実績: 9編で計74件の指摘が出て、**実際の誤りは3件だけ**だった
  （8編ぶんの41件は全件が上の既知型の誤検知。「いとしい人」の33件のうち3件が本物＝
  「二割」を「二十パーセント」に、「半年」を「六か月」に、否定の落ちを1箇所）。
  check-consistency 側も、犬の名「ヴューン」を「イヴァーン」の、「カーシャ」を「ミーシャ」の表記ゆれと疑い、
  「イエズス会的」（иезуитский）を「イエス」の、「テーブル」を「ルーブル」の表記ゆれと疑う。
  近似ペアの指摘は語形が似ているだけの別語を必ず含むので、台帳と突き合わせて落とすこと
...
- **本文コンテナ（`<div class="text">` / `prp-pages-output`）を持たない版がある**（チェーホフ短編の多く）。
  そのときは Wikisource 標準の `class="ws-noexport"` が非本文の目印になり、**PD ライセンス文もこの中**にある。
  落とさないと「Если произведение является переводом…」が第1段落として本文に混ざる。
  wikisource-to-paragraphs.ts が入れ子を数えて div / table ごと落とす（2026-09 追加）。
  **新しい版元では必ず先頭段落を見ること**（9編のうち6編がこれで汚染されていた）
- **末尾の注記の節が `references` を使わず「見出し＋素の `<p>`」で置かれる版がある**。
  `Примечания` / `См. также` / `Редакции` 等の既知の節名の見出しで切る（作品内の節番号を
  見出しに置く版＝狂人日記の 一〜十三 を切らないよう、既知の節名だけを見ている）
- **ru.wikisource は作品名の素のタイトルに「版の曖昧さ回避ページ」を置いていることがある**
  （Толстый и тонкий / Хамелеон / Ванька）。変換結果が **0段落**になったらまずこれを疑う。
  `Толстый и тонкий (Чехов)/ПСС 1975` のような本文ページを取り直すこと。
  同名別作品もある（Ванька は 1884年版と 1886年版があり、有名なのは 1886年版）
- **`source/paragraphs/stationmaster-01.src.json` は手編集済みで、コンバータでは再現できない。**
  題辞の出典（`Князь Вяземский.`）が `<td>` 直下の `<span>` にあって `<p>` 走査に入らないため、
  再生成すると 37→36 段落になり、詩句の改行も `\n` に変わる。**この章は再生成しないこと**
  （注釈・アウトラインが段落IDを参照している）
