...
- チェーホフ短編集（`chekhov`, ru）を開始（2026-09-12）。**初の「短編集＝1作品」構成**。1 workId に短編を
  章として並べるので、書棚には背表紙が1本だけ増え、構造ビューがそのまま目次になる（`chapterLabel` に短編名）。
  第1編「悲しみ」(Тоска, 1886) を追加: 62段落・原文1,464語 → 日本語4,935字（文庫約10ページ）、
  文対訳210セグメント、アウトライン（節6・段落要約16、machine-checked、FAIL 0）、
  読解支援レイヤー（語注31・人物3・参照1）、glossary 31件すべて `verified: false`。全段落 `draft`。
  PDは没1904で全法域クリア（戦時加算の検討も不要）。底本は ПСС в 30 томах, т.4（Наука, 1976）に拠る
  ru.wikisource rev 5644347。
  **訳の要点**: 原文は全編が現在形で書かれており、これが場面を目の前で進行させる仕掛けなので
  訳文も現在形で通す（style-guide の rules 3番目）。`горбач` は「背の曲がった男」で統一（「せむし」は
  現代日本語では差別語なので使わない）。二人称は ты→「おまえ」で固定した。
  次工程は短編の追加（候補: 役人の死・カメレオン・ワーニカ・かわいい女）と、glossary の裏取り＋
  `verified: true` 化（要確認8件＝霊歌『ヨセフの嘆き』の本文／辻馬車の借り賃と稼ぎ／ヴィボルグ地区の性格／
  頭巾つき外套の階級／ポリツェイスキー橋の現名／1880年代の辻馬車の相場／門番の届出義務の根拠／
  御者宿の実態／«спираль» の解釈）
- **短編集を1冊として扱うときの制約**: 底本（`sourceRef`）は works.json の作品単位にしかなく、章スキーマに無い。
  編ごとに Wikisource のページ・リビジョンが違うので、当面は `sourceRef` に編ごとの底本を列挙する。
  編数が増えたら章スキーマに任意の `sourceRef` を足すほうが安い
- 作品の長さの見積もり: 露語1語 ≒ 日本語3.29字（罪と罰の実測 176,000語→580,000字。
  チェーホフ「悲しみ」1,464語→4,935字＝3.37字で再現性を確認）、
...
- check-alignment の既知の誤検知: 複合数詞（семьсот тридцать 等）、日本語の自然な文分割による文数乖離（大半は良性）、
  **訳文の撥音便の否定**（「知らん」「分からん」「着かん」。jaNegations が ない/ぬ/ず しか見ないので
  「否定の消失」として出る。会話の多い章ほど増える）、`пока не …`（「〜するまで」構文。露語側に не が2つ出るが
  日本語に否定は現れない）
...
- **題辞が `<div class="epigraph">` に置かれる版がある**（チェーホフの «Кому повем…»）。`<p>` 走査から漏れるので
  wikisource-to-paragraphs.ts が入れ子 div を数えて `<center>` 相当のブロックに畳む（2026-09 追加）。
  題辞は本文の一部なので独立段落として訳す（駅長も p001＝詩句・p002＝出典の形）
- **本文の後ろに脚注・編集者注の一覧（`Примечания редакторов Викитеки`）が付く版がある**。
  注の中身に `<div class="poem"><p>` が入っていると本文の段落として拾ってしまう（題辞の出典の詩が
  最終段落として紛れこむ）。`class="references` / `mw-references-wrap` で切っている。
  末尾の段落が本文の最終行になっているか毎回目視すること
